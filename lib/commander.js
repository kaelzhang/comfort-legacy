...
mix(Commander.prototype, {
    cli: function() {
        ...
        if(data.options.err){
            ...
            this.run(data.command, data.options.parsed);
        }
    },
    ...
    run: function(command, options, callback) {
        var commander = this._get_commander(command);

        commander && commander.run(options, callback || this._callback_handler(command) );
    },

    // @returns {Object|false}
    _get_commander: function (command) {
        ...
        if(!commander){
            ...
            this.__commander[command] = commander;
        }
